...
public class CartService {
	...
	public ResponseEntity<Object> addProductToCart(String username, String sku) {
		try {
			...
			if(cartProductRepository.existsByCartCustomerPkAndProductSkuCode(customer.getPk(), sku)) {
				return ResponseEntity.status(409).body("The product already added in cart : " + sku);
			}
			...
			cart.setCartDiscountPrice(cart.updateCartDiscountPrice(cartProducts));
	...
	public ResponseEntity<Object> updateCart(String username, String sku, Integer quantity) {
		try {
			...
			cart.setCartDiscountPrice(cart.updateCartDiscountPrice(cart.getCartProducts()));
...
//	private double getCartDiscountOfProduct(ProductModel product, int quantity) {
//		
//		double discount = product.getDiscount();	
//		
//		return (discount==0)?0:discount*quantity;
//	}
